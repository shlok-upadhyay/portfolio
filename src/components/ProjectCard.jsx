    ...
    <div className="group w-[300px] sm:w-[430px] h-[470px] sm:h-[450px] flex flex-col  border border-gray-300 rounded-[30px]">
      <div style={{'--image-url': `url(${LightBg})`}} className="relative bg-cover w-full h-[300px] rounded-[30px] flex items-center justify-center bg-[#fdf7f4] xl:bg-[image:var(--image-url)] xl:bg-[110%] xl:bg-no-repeat overflow-hidden">
        ...
        <div className="flex gap-4">
          {(project.link) ? (
            <Link to={project.link} className="bg-[#251f2a] hover:bg-[#362d3d] text-white text-2xl w-[54px] h-[54px] rounded-full flex justify-center items-center scale-0 opacity-0 group-hover:scale-100 group-hover:opacity-100 transition-all duration-200">
              {(project.name === 'Hotel Management System') ? 
              <LuVideo /> : <LuLink2 />
              }
            </Link>
          ) : null}
          <Link to={project.github} className="bg-[#251f2a] text-white text-2xl w-[54px] h-[54px] rounded-full flex justify-center items-center scale-0 opacity-0 group-hover:scale-100 group-hover:opacity-100 transition-all duration-200">
            <LuGithub />
          </Link>
        </div>
      </div>
